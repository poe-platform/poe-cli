...
/***/ "./shared/conversation-layout.cjs":
/*!****************************************!*\
  !*** ./shared/conversation-layout.cjs ***!
  \****************************************/
/***/ ((module) => {
...
module.exports = { findLastUserIndex };


/***/ }),

/***/ "./vscode-extension/src/webview/components/settings-panel.ts":
/*!*******************************************************************!*\
  !*** ./vscode-extension/src/webview/components/settings-panel.ts ***!
  \*******************************************************************/
...
exports.PoeSettingsPanel = void 0;
exports.registerPoeSettingsPanel = registerPoeSettingsPanel;
...
class PoeSettingsPanel extends lit_1.LitElement {
    constructor() {
        super(...arguments);
        this.open = false;
        this.providers = [];
        ...
        this.strategyType = "fixed";
        this.strategyInfo = "";
        this._modelInput = "";
        this._fixedModelInput = "";
    }
    connectedCallback() {
        super.connectedCallback();
    ...
    handleFixedModelInput(event) {
        const target = event.target;
        this._fixedModelInput = target.value;
    }
    handleFixedModelCommit(event) {
        event.preventDefault();
    ...
    renderModelOption(option) {
        ...
        const baseClasses = "flex h-full w-full flex-col rounded-xl border border-border bg-surface px-4 py-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent";
    ...
    renderStrategyOption(option) {
        const isActive = option.id === this.strategyType;
        const baseClasses = "flex h-full w-full flex-col gap-2 rounded-xl border border-border bg-surface px-4 py-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent";
        const className = isActive
      ...
      >
        <div class="flex items-center justify-between">
          ...
          ${isActive
            ? (0, lit_1.html) `<span class="rounded-full bg-accent px-2 py-0.5 text-xs font-semibold text-accent-fg">Selected</span>`
            : null}
        </div>
      ...
      <section class="flex flex-col gap-8 rounded-2xl border border-border bg-surface-raised p-6 shadow-panel">
        ...
        <section class="flex flex-col gap-4 rounded-2xl border border-border bg-surface p-5 shadow-sm">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            : (0, lit_1.html) `<p class="rounded-xl border border-dashed border-border bg-surface-raised px-4 py-3 text-sm text-text-muted">
        ...
        <section class="flex flex-col gap-4 rounded-2xl border border-border bg-surface p-5 shadow-sm">
          ...
          ${this.strategyInfo
            ? (0, lit_1.html) `<p class="rounded-xl border border-border bg-surface px-4 py-2 text-sm text-text">
...
exports.PoeSettingsPanel = PoeSettingsPanel;
PoeSettingsPanel.properties = {
...
PoeSettingsPanel.styles = (0, lit_1.css) `
    :host {
      display: block;
      pointer-events: none;
    }
  ...
  `;
function registerPoeSettingsPanel(registry) {
    if (!registry) {
        return;
    }
...
registerPoeSettingsPanel(globalThis.customElements ?? null);


/***/ }),

/***/ "./vscode-extension/src/webview/runtime.ts":
/*!*************************************************!*\
  !*** ./vscode-extension/src/webview/runtime.ts ***!
  \*************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.initializeWebviewApp = initializeWebviewApp;
const settings_panel_js_1 = __webpack_require__(/*! ./components/settings-panel.js */ "./vscode-extension/src/webview/components/settings-panel.ts");
const shared_utils_1 = __webpack_require__(/*! @poe/shared-utils */ "./shared/conversation-layout.cjs");
function initializeWebviewApp(options) {
    const doc = options.document;
    const view = doc.defaultView ?? globalThis;
    const hostRegistry = view?.customElements ?? null;
    const globalRegistry = globalThis.customElements;
    (0, settings_panel_js_1.registerPoeSettingsPanel)(globalRegistry ?? null);
    (0, settings_panel_js_1.registerPoeSettingsPanel)(hostRegistry ?? null);
    ...
    const appShellHost = doc.querySelector("[data-slot='app-shell']");
    ...
    const sendButton = doc.getElementById("send-button");
    const stopButton = doc.getElementById("stop-button");
    const thinkingIndicator = doc.getElementById("thinking-indicator");
    const toolNotifications = doc.getElementById("tool-notifications");
    ...
    const chatHistoryContent = chatHistory?.querySelector(".chat-history-content");
    const historyCloseButton = chatHistory?.querySelector("[data-action='history-close']");
    if (appShellHost) {
        appShellHost.innerHTML = options.appShellHtml;
    }
    const navNewChat = appShellHost?.querySelector("[data-action='new-chat']") ?? null;
    const navHistory = appShellHost?.querySelector("[data-action='chat-history']") ?? null;
    const navSettings = appShellHost?.querySelector("[data-action='open-settings']") ?? null;
    const welcomeSnapshot = messagesDiv?.innerHTML ??
    ...
    function insertTimelineEntry(entry, index) {
    ...
    function rebuildTimelineFromDom() {
        clearTimeline();
        if (!messagesDiv) {
            return;
        }
        const wrappers = Array.from(messagesDiv.querySelectorAll(".message-wrapper"));
    ...
    const uiClasses = {
        messageWrapperBase: "message-wrapper flex flex-col gap-3 rounded-2xl border border-border bg-surface-raised p-4 shadow-panel transition",
        messageWrapperUser: "bg-surface",
        messageWrapperAssistant: "bg-surface-raised",
        messageHeader: "message-header flex items-center gap-3 text-sm font-semibold text-text",
        messageModel: "message-model ml-auto text-xs font-medium text-text-muted",
        avatar: "avatar flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-surface text-xs font-semibold text-text",
        avatarAssistant: "bg-surface-raised overflow-hidden",
        avatarUser: "uppercase bg-surface text-text",
        messageContent: "message-content space-y-3 text-sm leading-6 text-text [&_a]:text-accent [&_a]:underline [&_code]:font-mono [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:border [&_pre]:border-border [&_pre]:bg-surface [&_pre]:px-3 [&_pre]:py-2",
        diffWrapper: "message-wrapper diff flex flex-col gap-3 overflow-hidden rounded-2xl border border-border bg-surface shadow-panel",
        diffContent: "message-content text-sm leading-6 text-text [&_pre]:border-0 [&_pre]:bg-transparent",
        errorWrapper: "message-wrapper error flex flex-col gap-3 rounded-2xl border border-error bg-surface p-4 text-error shadow-panel",
        toolWrapper: "message-wrapper tool flex flex-col gap-3 rounded-2xl border border-dashed border-border bg-surface px-4 py-3 shadow-md transition",
        toolWrapperSuccess: "border-success",
        toolWrapperError: "border-error bg-surface",
        toolHeader: "message-header flex items-center gap-3 text-sm font-semibold text-text",
        toolIcon: "tool-icon flex h-8 w-8 items-center justify-center rounded-xl bg-surface-raised text-base",
        ...
        toolArgs: "message-tool-args rounded-xl bg-surface-raised px-3 py-2 text-xs font-mono text-text",
        toolResponse: "message-tool-response rounded-xl bg-surface-raised px-3 py-2 text-xs font-mono text-text",
        toolError: "message-tool-error text-xs text-error",
        toolNotification: "tool-notification pointer-events-auto rounded-xl border border-border bg-surface-raised px-4 py-2 text-sm font-medium text-text shadow-md transition-opacity duration-200",
        ...
        chatHistoryItem: "chat-history-item cursor-pointer rounded-xl border border-border bg-surface p-4 transition hover:bg-surface-raised",
        ...
        chatHistoryEmpty: "chat-history-empty rounded-xl border border-dashed border-border bg-surface px-4 py-6 text-sm text-text-muted text-center",
    };
    ...
    function applyNavStyles(button, isActive, variant) {
        if (!button) {
            return;
        }
    ...
    function renderAvatar(role) {
        ...
        avatar.className = joinClasses(uiClasses.avatar, role === "assistant" ? uiClasses.avatarAssistant : uiClasses.avatarUser);
        if (role === "assistant") {
    ...
    function addMessageHtml(html, role, model) {
        if (!messagesDiv) {
            return;
        }
        ...
        wrapper.className = joinClasses(uiClasses.messageWrapperBase, role === "assistant" ? uiClasses.messageWrapperAssistant : uiClasses.messageWrapperUser, role);
    ...
    function rememberToolMessage(toolName, element) {
    ...
    function consumeToolMessage(toolName) {
    ...
    function addToolMessage(details) {
        if (!messagesDiv) {
            return;
        }
        const welcome = messagesDiv.querySelector(".welcome-message");
        ...
        if (details.args && typeof details.args === "object") {
            const entries = Object.entries(details.args);
            if (entries.length > 0) {
        ...
        wrapper.appendChild(content);
        const entry = { role: "tool", element: wrapper };
        const lastUserIndex = (0, shared_utils_1.findLastUserIndex)(conversationTimeline);
        if (lastUserIndex >= 0) {
    ...
    function completeToolMessage(details) {
        ...
        entry.className = joinClasses(uiClasses.toolWrapper, details.success ? uiClasses.toolWrapperSuccess : uiClasses.toolWrapperError, details.success ? "success" : "error");
        entry.dataset.toolName = details.toolName;
        const statusLine = entry.querySelector(".message-tool-status");
        if (statusLine) {
            statusLine.className = joinClasses(uiClasses.toolStatus, details.success ? uiClasses.toolStatusSuccess : uiClasses.toolStatusError);
            if (details.success) {
    ...
    function updateStrategyBadge(info, enabled) {
        if (!settingsPanel) {
            return;
        }
    ...
    function deriveStrategyType(info) {
    ...
    function showToolNotification(text, variant) {
        if (!toolNotifications) {
            return;
        }
        const item = doc.createElement("div");
        const variantClass = variant === "running"
        ...
        handleMessage(rawMessage) {
            ...
            switch (type) {
                ...
                case "toolExecuted": {
                    if (typeof message.toolName === "string") {
                        completeToolMessage({
                            ...
                            error: typeof message.error === "string" && message.error.length > 0
                                ? message.error
                                : undefined
                        });
                        if (message.success) {
                ...
                case "taskComplete": {
                    ...
                    const summary = typeof task.result === "string"
                        ? task.result
                        : typeof task.error === "string"
                            ? task.error
                            : "Task finished.";
                    const toolLabel = typeof task.toolName === "string" && task.toolName.length > 0
                        ? task.toolName
                        : "Task";
                    const success = task.status === "completed";
                    showToolNotification(`${success ? "✅" : "❌"} ${toolLabel}: ${summary}`, success ? "success" : "error");
                    break;
                }
                case "error": {
                    toggleThinking(false);
...
/*!*******************************************************!*\
  !*** ./vscode-extension/src/webview/preview/entry.ts ***!
  \*******************************************************/

Object.defineProperty(exports, "__esModule", ({ value: true }));
const runtime_1 = __webpack_require__(/*! ../runtime */ "./vscode-extension/src/webview/runtime.ts");
__webpack_require__(/*! ../styles/tailwind.css */ "./vscode-extension/src/webview/styles/tailwind.css");
window.initializeWebviewApp = runtime_1.initializeWebviewApp;
