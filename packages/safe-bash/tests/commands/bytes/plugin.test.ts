...
const names = ["base64", "base32", "xxd", "od", "sha512sum", "sha384sum", "sha256sum", "sha224sum", "sha1sum", "md5sum", "cksum", "gzip", "gunzip", "zcat"];
...
test("byte factory returns all fourteen unique commands in family order", () => {
  const definitions = createByteCommands();
  assert.deepEqual(definitions.map((definition) => definition.name), names);
  assert.equal(new Set(definitions.map((definition) => definition.name)).size, 14);
...
test("replacement is explicit for every command and preserves unrelated registrations", async () => {
  ...
  assert.equal(commands.list().length, 15);
