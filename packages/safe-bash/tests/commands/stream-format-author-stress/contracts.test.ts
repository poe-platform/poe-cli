...
test("default factory contains96 and standalone formatting installs exactly four without split", async () => {
  const instance = new Shell({ fs: createMemoryFileSystem() }).use(agentCommands());
  assert.equal(createAgentCommands().length, 96);
  ...
  assert.equal(instance.commands.list().length, 96);
