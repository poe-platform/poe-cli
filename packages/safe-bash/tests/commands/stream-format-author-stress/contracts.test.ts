...
test("default factory contains84 and standalone formatting installs exactly four without split", async () => {
  const instance = new Shell({ fs: createMemoryFileSystem() }).use(agentCommands());
  assert.equal(createAgentCommands().length, 84);
  ...
  assert.equal(instance.commands.list().length, 84);
