...
test("default factory contains95 and standalone formatting installs exactly four without split", async () => {
  const instance = new Shell({ fs: createMemoryFileSystem() }).use(agentCommands());
  assert.equal(createAgentCommands().length, 95);
  ...
  assert.equal(instance.commands.list().length, 95);
