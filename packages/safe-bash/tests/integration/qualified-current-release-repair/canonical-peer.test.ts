...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 89);
  assert.equal(new Set(expectedCurrentCommands).size, 89);
...
test("generated current stream catalog accepts the independent exact 89 names at all three boundaries", async () => {
