...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 95);
  assert.equal(new Set(expectedCurrentCommands).size, 95);
...
test("generated current stream catalog accepts the independent exact 95 names at all three boundaries", async () => {
