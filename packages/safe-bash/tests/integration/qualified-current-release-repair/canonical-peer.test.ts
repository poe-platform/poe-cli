...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 86);
  assert.equal(new Set(expectedCurrentCommands).size, 86);
...
test("generated current stream catalog accepts the independent exact 86 names at all three boundaries", async () => {
