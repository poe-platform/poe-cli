...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 93);
  assert.equal(new Set(expectedCurrentCommands).size, 93);
...
test("generated current stream catalog accepts the independent exact 93 names at all three boundaries", async () => {
