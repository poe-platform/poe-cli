...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 87);
  assert.equal(new Set(expectedCurrentCommands).size, 87);
...
test("generated current stream catalog accepts the independent exact 87 names at all three boundaries", async () => {
