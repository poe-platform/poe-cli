...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 92);
  assert.equal(new Set(expectedCurrentCommands).size, 92);
...
test("generated current stream catalog accepts the independent exact 92 names at all three boundaries", async () => {
