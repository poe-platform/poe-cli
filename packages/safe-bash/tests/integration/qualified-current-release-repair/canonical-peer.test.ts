...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 85);
  assert.equal(new Set(expectedCurrentCommands).size, 85);
...
test("generated current stream catalog accepts the independent exact 85 names at all three boundaries", async () => {
