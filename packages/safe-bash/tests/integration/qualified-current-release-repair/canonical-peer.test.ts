...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 94);
  assert.equal(new Set(expectedCurrentCommands).size, 94);
...
test("generated current stream catalog accepts the independent exact 94 names at all three boundaries", async () => {
