...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 96);
  assert.equal(new Set(expectedCurrentCommands).size, 96);
...
test("generated current stream catalog accepts the independent exact 96 names at all three boundaries", async () => {
