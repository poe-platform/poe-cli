...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 91);
  assert.equal(new Set(expectedCurrentCommands).size, 91);
...
test("generated current stream catalog accepts the independent exact 91 names at all three boundaries", async () => {
