...
async function generatedCatalogGuards() {
  ...
  assert.equal(expectedCurrentCommands.length, 82);
  assert.equal(new Set(expectedCurrentCommands).size, 82);
...
test("generated current stream catalog accepts the independent exact 82 names at all three boundaries", async () => {
