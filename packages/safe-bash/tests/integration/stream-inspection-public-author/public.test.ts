...
test("root family exports preserve four inspection definitions in the 87-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 87);
  assert.deepEqual(definitions.slice(64, 68), names);
  assert.deepEqual(definitions.slice(68), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch"]);
  const target = host();
