...
test("root family exports preserve four inspection definitions in the 89-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 89);
  assert.deepEqual(definitions.slice(64, 68), names);
  assert.deepEqual(definitions.slice(68), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch", "xq", "xmllint"]);
  const target = host();
