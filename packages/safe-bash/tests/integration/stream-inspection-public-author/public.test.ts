...
test("root family exports preserve four inspection definitions in the 94-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 94);
  assert.deepEqual(definitions.slice(66, 70), names);
  assert.deepEqual(definitions.slice(70), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch", "xq", "xmllint", "csplit", "pr", "tsort"]);
  const target = host();
...
test("standalone and aggregate share one explicit replacement boundary", async () => {
  ...
  assert.equal(target.commands.list().length, 95);
