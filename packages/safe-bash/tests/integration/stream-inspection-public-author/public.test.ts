...
test("root family exports preserve four inspection definitions in the 84-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 84);
  assert.deepEqual(definitions.slice(59, 63), names);
  assert.deepEqual(definitions.slice(63), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch", "xq", "xmllint"]);
  const target = host();
...
test("standalone and aggregate share one explicit replacement boundary", async () => {
  ...
  assert.equal(target.commands.list().length, 85);
