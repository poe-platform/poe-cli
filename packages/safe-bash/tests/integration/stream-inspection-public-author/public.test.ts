...
test("root family exports preserve four inspection definitions in the 85-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 85);
  assert.deepEqual(definitions.slice(62, 66), names);
  assert.deepEqual(definitions.slice(66), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch"]);
  const target = host();
