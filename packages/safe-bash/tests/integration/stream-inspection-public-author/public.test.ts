...
test("root family exports preserve four inspection definitions in the 86-command aggregate", async () => {
  ...
  assert.equal(definitions.length, 86);
  assert.equal(new Set(definitions).size, 86);
  assert.deepEqual(definitions.slice(63, 67), names);
  assert.deepEqual(definitions.slice(67), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch"]);
  const target = host();
