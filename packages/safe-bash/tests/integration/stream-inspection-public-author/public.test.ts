...
test("root family exports preserve four inspection definitions in the 83-command aggregate", async () => {
  ...
  assert.equal(new Set(definitions).size, 83);
  assert.deepEqual(definitions.slice(60, 64), names);
  assert.deepEqual(definitions.slice(64), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch"]);
  const target = host();
