...
test("root family exports preserve four inspection definitions in the 84-command aggregate", async () => {
  ...
  assert.deepEqual(definitions.slice(61, 65), names);
  assert.deepEqual(definitions.slice(65), ["seq", "nl", "rev", "unexpand", "split", "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch"]);
  const target = host();
