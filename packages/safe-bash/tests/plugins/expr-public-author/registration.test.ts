...
test("public93 inventory retains frozen expr76 plus which, timeout, apply_patch, three SHA-2 commands, cmp, fmt, shuf, numfmt, truncate, XML queries, zip, unzip, csplit and pr", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names93 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum", "cmp", "fmt", "shuf", "numfmt", "truncate", "xq", "xmllint", "zip", "unzip", "csplit", "pr"].sort();
  assert.equal(names93.length, 93); assert.equal(new Set(names93).size, 93);
  assert.deepEqual(createAgentCommands().map(command => command.name).sort(), names93);
  const target = host(); await agentCommands().setup(target);
  assert.deepEqual(target.commands.list().map(command => command.name).sort(), names93);
...
test("expr aggregate collision preflight and top-level replacement preserve custom commands", async () => {
  ...
  assert.equal(target.commands.list().length, 94); assert.equal(target.commands.get("custom"), registeredCustom); assert.notEqual(target.commands.get("expr"), registeredExpr);
});
