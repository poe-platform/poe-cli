...
test("public84 inventory retains frozen expr76 plus which, timeout, apply_patch, three SHA-2 commands and XML queries", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names84 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum", "xq", "xmllint"].sort();
  assert.equal(names84.length, 84); assert.equal(new Set(names84).size, 84);
  assert.deepEqual(createAgentCommands().map(command => command.name).sort(), names84);
  const target = host(); await agentCommands().setup(target);
  assert.deepEqual(target.commands.list().map(command => command.name).sort(), names84);
...
test("expr aggregate collision preflight and top-level replacement preserve custom commands", async () => {
  ...
  assert.equal(target.commands.list().length, 85); assert.equal(target.commands.get("custom"), registeredCustom); assert.notEqual(target.commands.get("expr"), registeredExpr);
});
