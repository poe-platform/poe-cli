...
test("public82 inventory retains frozen expr76 plus which, timeout, apply_patch and three SHA-2 commands", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names82 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum"].sort();
  assert.equal(names82.length, 82); assert.equal(new Set(names82).size, 82);
  assert.deepEqual(createAgentCommands().map(command => command.name).sort(), names82);
  const target = host(); await agentCommands().setup(target);
  assert.deepEqual(target.commands.list().map(command => command.name).sort(), names82);
...
test("expr aggregate collision preflight and top-level replacement preserve custom commands", async () => {
  ...
  assert.equal(target.commands.list().length, 83); assert.equal(target.commands.get("custom"), registeredCustom); assert.notEqual(target.commands.get("expr"), registeredExpr);
});
