...
test("public83 inventory retains frozen expr76 plus which, timeout, apply_patch three SHA-2 commands and cmp", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names83 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum", "cmp"].sort();
  assert.equal(names83.length, 83); assert.equal(new Set(names83).size, 83);
  assert.deepEqual(createAgentCommands().map(command => command.name).sort(), names83);
  const target = host(); await agentCommands().setup(target);
  assert.deepEqual(target.commands.list().map(command => command.name).sort(), names83);
...
test("expr aggregate collision preflight and top-level replacement preserve custom commands", async () => {
  ...
  assert.equal(target.commands.list().length, 84); assert.equal(target.commands.get("custom"), registeredCustom); assert.notEqual(target.commands.get("expr"), registeredExpr);
});
