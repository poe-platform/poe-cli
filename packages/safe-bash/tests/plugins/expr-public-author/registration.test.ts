...
test("public84 inventory retains frozen expr76 plus which, timeout, apply_patch three SHA-2 commands, cmp and fmt", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names84 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum", "cmp", "fmt"].sort();
