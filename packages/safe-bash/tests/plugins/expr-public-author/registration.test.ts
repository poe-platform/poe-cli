...
test("public89 inventory retains frozen expr76 plus which, timeout, apply_patch, three SHA-2 commands, cmp, fmt, shuf, numfmt, truncate and XML queries", async () => {
  assert.equal(expected.names76.length, 76); assert.equal(new Set(expected.names76).size, 76);
  const names89 = [...expected.names76, "which", "timeout", "apply_patch", "sha512sum", "sha384sum", "sha224sum", "cmp", "fmt", "shuf", "numfmt", "truncate", "xq", "xmllint"].sort();
  assert.equal(names89.length, 89); assert.equal(new Set(names89).size, 89);
  assert.deepEqual(createAgentCommands().map(command => command.name).sort(), names89);
  const target = host(); await agentCommands().setup(target);
  assert.deepEqual(target.commands.list().map(command => command.name).sort(), names89);
...
test("expr aggregate collision preflight and top-level replacement preserve custom commands", async () => {
  ...
  assert.equal(target.commands.list().length, 90); assert.equal(target.commands.get("custom"), registeredCustom); assert.notEqual(target.commands.get("expr"), registeredExpr);
});
