...
test("default inventory matches the independent 96 names and excludes host opt-ins", () => {
  const names = createAgentCommands().map(command => command.name);
  assert.equal(names.length, 96);
  assert.deepEqual(names.sort(), expected);
  assert.equal(new Set(names).size, 96);
});
...
test("portable registration is atomic and replacement preserves unrelated commands", async () => {
  ...
  assert.equal(commands.list().length, 97);
