...
test("current registry is frozen60 plus twenty-nine independently declared delivered commands", async () => {
  ...
  const expected = [...baseline60.flatMap(name => name === "find" ? ["find", "cmp", "fmt", "shuf", "numfmt"] : name === "sha256sum" ? ["sha512sum", "sha384sum", "sha256sum", "sha224sum"] : name === "mktemp" ? ["mktemp", "truncate"] : [name]), ...approved, "date", "sleep", "printenv", "tree", "file", "egrep", "fgrep", "column", "html-to-markdown", "du", "expr", "which", "timeout", "apply_patch", "xq", "xmllint"];
  assert.equal(expected.length, 89);
  assert.equal(new Set(expected).size, 89);
...
for (const name of approved) test(`${name} aggregate collision is atomic and replacement remains explicit`, async () => {
  ...
  assert.equal(target.commands.list().length, 90);
