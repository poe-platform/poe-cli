...
export function createFloat32ArrayPrototypes(budget: Budget, constructor: SandboxClosure): void {
  ...
  for (const key of ["set", "slice", "subarray", "fill", "copyWithin", "reverse", "at", "includes"])
...
export function getFloat32Member(
  ...
  if (!["set", "slice", "subarray", "fill", "copyWithin", "reverse", "at", "includes"].includes(key)) return undefined;
  return createSandboxClosure({
    guest: true,
    sandbox: true,
    name: key,
    length: key === "reverse" ? 0 : key === "set" || key === "fill" || key === "at" || key === "includes" ? 1 : 2,
    call: (args, context) => {
      const receiver = context?.thisValue;
      if (!isFloat32Array(receiver))
        throw new TypeError(`Float32Array#${key} requires a Float32Array receiver.`);
      const storage = float32Storage(receiver, key === "slice" || key === "fill" || key === "copyWithin" || key === "reverse" || key === "at" || key === "includes");
      if (key === "reverse") {
        ...
        return (async () => {
          ...
          try {
            ...
            for (let index = start; index < storage.length; index++) {
              ...
              if (element === args[0] || (Number.isNaN(element) && Number.isNaN(args[0]))) return true;
            }
            return false;
          } finally { release(); }
        })();
